...
Section semantics.
  ...
  Definition sem_arrow P Q t := ∀u, P u → Q (t @ u). 
  ...
  Notation "P ≡ Q" := (∀u, P u ↔ Q u) (at level 70).
  ...
  Local Definition Nsaturated P := ∀ u a l, N a → P (u⌈a⌉@*l) → P ((Ⲗ u@a)@*l).
  ...
  Local Fact Nsaturated_cap X (Q : X → Prop) (f : X → term → Prop) :
         (∀x, Q x → Nsaturated (f x))
       → Nsaturated (λ u, ∀x, Q x → f x u).
  Proof. intros H u a l H1 H2 ? ?; apply H; eauto. Qed. 

  Local Definition N0 u := ∃ x l, u = £x @* l ∧ Forall N l.
  ...
  Fixpoint type_sem (A : type) (I : nat → term → Prop) : term → Prop :=
    ...
    | ∇ A => λ u, ∀P, Nmodel P → type_sem A (P∷I) u
    end.

  Hint Resolve in_or_app : core.

  Fact type_sem_ext A I J : (∀x, x ∈ 𝓥 A → I x ≡ J x) → type_sem A I ≡ type_sem A J. 
  Proof.
  ...
  Fact type_sem_Nmodel A I : 
         (∀x, x ∈ 𝓥 A → Nmodel (I x)) → Nmodel (type_sem A I).
  Proof.
  ...
  Fact type_sem_ren A I f : type_sem A⟬f⟭ I ≡ type_sem A (I⋄f).
  Proof.
  ...
  Fact type_sem_subst A I f : type_sem A⟪f⟫ I ≡ type_sem A (λ x, type_sem (f x) I).
  Proof.
    induction A as [ x | A IHA B IHB | A IHA ] in I, f |- *; simpl; intros u; try easy.
    + apply sem_arrow_congr; auto.
    + split.
  ...
  Theorem FTJ_adequacy Γ u A :
        Γ ⊢ u ∶ A
      → ∀ I f,
            (∀x, (* x ∈ syn_vars A \/ x ∈ syn_vars (Γ n) → *) Nmodel (I x))
          → (∀n, n ∈ 𝓥 u → type_sem (Γ n) I (f n))
          → type_sem A I u⟪f⟫.
  Proof.
